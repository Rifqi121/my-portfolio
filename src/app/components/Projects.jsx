...
<section id="projects" className="h-auto bg-gradient-to-b from-gray-800 to-gray-900 text-white relative">
  ...
  className="rounded-lg w-full h-[20vh] object-cover mb-4" />
  ...
  <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4">
    <div
      className="modal-content bg-gray-800 p-6 rounded-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto flex flex-col relative">
      <button onClick={()=> setSelectedProject(null)} className="absolute top-4 right-4 text-white text-2xl">✖</button>
      ...
      <div className="mt-4">
        <Swiper spaceBetween={10} slidesPerView={1} autoplay={{ delay: 2000, disableOnInteraction: false }}
          ...
          <SwiperSlide key={i}>
            <Image src={src} alt={`Screenshot ${i + 1}`} width={500} height={300}
      ...
      <a href={selectedProject.link} target="_blank" rel="noopener noreferrer"
        className="block mt-4 px-4 py-2 text-center bg-blue-500 text-white rounded-lg">
        Go to Repository
      </a>
    </div>
  </div>
